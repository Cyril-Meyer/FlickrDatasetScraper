...
def scrap(tags=None, tag_mode='all', text=None, maximum=500, output_folder="out"):
    ...
    if tags is not None:
        photos = flickr.walk(tag_mode='all', tags=tags, per_page=500, license=config.license)
    else:
        photos = flickr.walk(text=text, per_page=500, license=config.license)

    for index, photo in zip(range(maximum), photos):
        print(f"{index:0{max_digits}d}", photo.get('title'))
        ...
        try:
            ...
            print(url)
            # save image
            with open(f"{output_folder}/{photo.get('id')}.jpg", 'wb') as file:
                file.write(img_data)
